...
render_session_header(6, "Streamlit", "2:45 PM", "15 min", "Operations dashboard with AI chat interface")
...
PROMPT_6_1 = """In Workspaces, create a Streamlit app called REVOPS_DASHBOARD in ADTECH_AI.REVOPS.

First, create a compute pool:
- Name: REVOPS_COMPUTE_POOL
...
- KPI row with st.metric: Total Platform Revenue (sum net_revenue), Active Campaigns (count where status='active'), Average ROAS (revenue/spend), Clients at Risk (count where retention_risk='high')
- Revenue by Tier bar chart (st.bar_chart): group revenue_monthly by advertiser tier
- Campaign Performance trend line chart (st.line_chart): daily impressions and clicks over time
- Top 10 Advertisers by Revenue table (st.dataframe): showing name, tier, total revenue, take rate
- Client Health distribution: bar chart showing count of advertisers by retention_risk level
...
- A chat interface using our REVOPS_AGENT via SNOWFLAKE.CORTEX.AGENT()
...
render_what_you_built([
    "REVOPS_COMPUTE_POOL — compute pool for the Streamlit app",
    "REVOPS_DASHBOARD — 2-page Streamlit app using built-in components",
    "Operations Dashboard with KPIs and charts (no external dependencies)",
    "AI-powered chat interface connected to REVOPS_AGENT",
])
